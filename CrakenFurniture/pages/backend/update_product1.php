...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    $sql = "SELECT image_data FROM products1 WHERE product_id = ?";
    ...
    if ($image_data === $current_image_data) {
        ...
        $sql = "UPDATE products1 SET product_name = ?, price = ?, description = ? WHERE product_id = ?";
        if ($stmt = $db->connection->prepare($sql)) {
            $stmt->bind_param('ssdi', $product_name, $price, $description, $product_id);
            if ($stmt->execute()) {
        ...
        $sql = "UPDATE products1 SET product_name = ?, price = ?, description = ?, image_data = ? WHERE product_id = ?";
        if ($stmt = $db->connection->prepare($sql)) {
            $stmt->bind_param('ssdsi', $product_name, $price, $description, $image_data, $product_id);
            if ($stmt->execute()) {
                echo "Product updated successfully with the new image.";
                header("Location: ../../?page=products1");
                exit;
            } else {
                echo "Error updating product: " . $stmt->error;
            }
