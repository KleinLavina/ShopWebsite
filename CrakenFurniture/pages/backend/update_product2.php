...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    $sql = "SELECT image_data FROM products2 WHERE product_id = ?";
    ...
    if ($image_data === $current_image_data) {
        ...
        $sql = "UPDATE products2 SET product_name = ?, price = ?, description = ? WHERE product_id = ?";
        if ($stmt = $db->connection->prepare($sql)) {
        ...
        $sql = "UPDATE products2 SET product_name = ?, price = ?, description = ?, image_data = ? WHERE product_id = ?";
        if ($stmt = $db->connection->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: ../../?page=products2");
                exit;
            } else {
                echo "Error updating product: " . $stmt->error;
            }
